...
class Test_algorithm_rscript(unittest.TestCase):
    def setup_test(self):
    ...
    def create_sdm_script(self, test_script, r_script):
        script = u'\n'.join([
            resource_string('org.bccvl.compute', 'rscripts/bccvl.R'),
            resource_string('org.bccvl.compute', 'rscripts/eval.R'),
            resource_string('org.bccvl.compute', 'tests/SampleMat2.R'),
            resource_string('org.bccvl.compute', r_script)])
        open(test_script, 'wb').write(script)

    def setup_paramaters(self, model_name, work_dir):
        ...
        # Setup the default values for model-specific parameters
        if model_name == 'ann':
            params["params"]['nbcv'] = 5
            params["params"]['rang'] = 0.1
            params["params"]['maxit'] = 200
        if model_name == 'cta':
            params["params"]['method'] = 'class'
            params["params"]['control_xval'] = 5
            params["params"]['control_minbucket'] = 5
            params["params"]['control_minsplit'] = 5
            params["params"]['control_cp'] = 0.001
            params["params"]['control_maxdepth'] = 25
        if model_name == 'fda':
            params["params"]['method'] = 'mars'
        if model_name == 'sre':
            params["params"]['quant'] = 0.025
        if model_name == 'gam':
            params["params"]['family'] = 'binomial'
            params["params"]['irls_reg'] = 0
            params["params"]['epsilon'] = 1e-07
            params["params"]['maxit'] = 200
            params["params"]['mgcv_tol'] = 1e-07
            params["params"]['mgcv_half'] = 15
        if model_name == 'gbm':
            params["params"]['distribution'] = 'bernoulli'
            params["params"]['n_trees'] = 2500
            params["params"]['interaction_depth'] = 7
            params["params"]['n_minobsinnode'] = 5
            params["params"]['shrinkage'] = 0.001
            params["params"]['bag_fraction'] = 0.5
            params["params"]['train_fraction'] = 1
            params["params"]['cv_folds'] = 3
        if model_name == 'glm':
            params["params"]['type'] = 'quadratic'
            params["params"]['interaction_level'] = 0
            params["params"]['test'] = 'AIC'
            params["params"]['family'] = 'binomial'
            params["params"]['mustart'] = 0.5
            params["params"]['control_epsilon'] = 1e-08
            params["params"]['control_maxit'] = 50
            params["params"]['control_trace'] = False
        if model_name == 'mars':
            params["params"]['degree'] = 2
            params["params"]['penalty'] = 2
            params["params"]['thresh'] = 0.001
            params["params"]['prune'] = True
        if model_name == 'rf':
            params["params"]['do.classif'] = True
            params["params"]['ntree'] = 500
            params["params"]['nodesize'] = 5
            params["params"]['mtry'] = 'default'
        if model_name == 'maxent':
            params["params"]['maximumiterations'] = 200
            params["params"]['linear'] = True
            params["params"]['quadratic'] = True
            params["params"]['product'] = True
            params["params"]['threshold'] = True
            params["params"]['hinge'] = True
            params["params"]['lq2lqptthreshold'] = 80
            params["params"]['l2lqthreshold'] = 10
            params["params"]['hingethreshold'] = 15
            params["params"]['beta_threshold'] = -1.0
            params["params"]['beta_categorical'] = -1.0
            params["params"]['beta_lqp'] = -1.0
            params["params"]['beta_hinge'] = -1.0
            params["params"]['defaultprevalence'] = 0.5
        if model_name == 'brt':
            params["params"]['tree_complexity'] = 1
            params["params"]['learning_rate'] = 0.01
            params["params"]['bag_fraction'] = 0.75
            params["params"]['n_folds'] = 10
            params["params"]['prev_stratify'] = True
            params["params"]['family'] = 'bernoulli'
            params["params"]['n_trees'] = 50
            params["params"]['max_trees'] = 10000
            params["params"]['tolerance_method'] = 'auto'
            params["params"]['tolerance_value'] = 0.001


        jfile = open(os.path.join(work_dir, 'params.json'), mode='wb')
        json.dump(params, jfile, indent=2)
    ...
    def run_test(self, model_name, md5_digests):
        # setup test directory
        temp_dir = self.setup_test()

        try:
            # Create R script
            self.create_sdm_script(os.path.join(temp_dir, 'test.R'), 'content/toolkit/' + model_name + '/' + model_name + '.R')

            # setup a jason parameter file for the test
            self.setup_paramaters(model_name, temp_dir)

            # Run the script
            proc = subprocess.Popen(["Rscript", "test.R"], cwd=temp_dir, close_fds=True, stderr=subprocess.STDOUT)
            ret = proc.wait()

            # Check the md5 digest
            success = self.validate_md5(md5_digests, temp_dir)
    ...
    def test_ann(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : '10173036b71a0f517133ae4e80aa2761', 
        ...
        self.run_test('ann', md5_digests)

    def test_sre(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'f59f276dce68e9eec22895f529a71f98', 
        ...
        self.run_test('sre', md5_digests)

    def test_cta(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'a4eb942b63418fc9792115e11952207a', 
        ...
        self.run_test('cta', md5_digests)

    def test_fda(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'ce40e19526e0d5ae75b397d55e9fc08b', 
        ...
        self.run_test('fda', md5_digests)


    def test_gam(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'a58e56e4b3f2fc07cad4ea1d84debe35', 
        ...
        self.run_test('gam', md5_digests)

    def test_gbm(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : '54e1d8732801b3694991256977b49078', 
        ...
        self.run_test('gbm', md5_digests)

    def test_glm(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'e1bdd6e457cd40c7c3464eb95c97ca04', 
        ...
        self.run_test('glm', md5_digests)

    def test_mars(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'c54b18d8baf87ee62b3961f1b831f095', 
        ...
        self.run_test('mars', md5_digests)

    def test_rf(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : 'f495735d5fe7d3a9f68b03fadd870180', 
        ...
        self.run_test('rf', md5_digests)

    def test_maxent(self):
        md5_digests =  {'biomod2.modelEvaluation.csv' : '7d9c4e0ec5fcaf0ee44ddca8d762439b', 
        ...
        self.run_test('maxent', md5_digests)

    def test_bioclim(self):
        md5_digests =  {'AUC.png' : 'aec420da80d33406eab0c0670c302603', 
        ...
        self.run_test('bioclim', md5_digests)

    def test_brt(self):
        md5_digests =  {'AUC.png' : 'daa94284788159870efe0a748df8a765', 
        ...
        self.run_test('brt', md5_digests)

    def test_circles(self):
        md5_digests =  {'proj_current_Rhinella.tif' : '2438901fd4b1414ebf0f23e86352a20f', 
                        'Rhinella.model.object.RData' : None}
        self.run_test('circles', md5_digests)

    def test_convHull(self):
        md5_digests =  {'proj_current_Rhinella.tif' : '896efaaded8035a41ec8b67b23af81b0', 
                         'Rhinella.model.object.RData' : None}
        self.run_test('convHull', md5_digests)

    def test_voronoiHull(self):
        md5_digests =  {'proj_current_Rhinella.tif' : '2438901fd4b1414ebf0f23e86352a20f', 
                        'Rhinella.model.object.RData' : None}
    ...
    def test_geoIDW(self):
        md5_digests =  {'proj_current_Rhinella.tif' : 'f817e6703142a870d5d0eef340c9cfe2', 
                         'Rhinella.model.object.RData' : None}
        self.run_test('geoIDW', md5_digests)
