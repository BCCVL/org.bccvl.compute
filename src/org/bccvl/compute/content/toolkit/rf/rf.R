...
species_algo_str = ifelse(is.null(bccvl.params$subset), 
                          sprintf("%s_rf", occur.species), 
                          sprintf("%s_rf_%s", occur.species, bccvl.params$subset))
...
bccvl.VIPplot(method="rf", data1=data1, pdf=TRUE, 
...
if (enviro.data.genUnconstraintMap &&
   ...
   (!is.null(enviro.data.constraints) || enviro.data.generateCHall)) {
    ...
    bccvl.grdtogtiff(file.path(getwd(),
                     ...
                     algorithm=ifelse(is.null(bccvl.params$subset), "rf", sprintf("rf_%s", bccvl.params$subset)),
