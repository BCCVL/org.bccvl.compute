...
biomod.modeling.id = bccvl.params$modeling_id # bccvl.params$species  #character, the ID (=name) of modeling procedure. A random number by default
...
if (enviro.data.genUnconstraintMap && 
   ...
   (!is.null(enviro.data.constraints) || enviro.data.generateCHall)) {
    ...
    bccvl.grdtogtiff(file.path(getwd(),
                     ...
                     algorithm=ifelse(is.null(bccvl.params$subset), "cta", sprintf("cta_%s", bccvl.params$subset)), 
