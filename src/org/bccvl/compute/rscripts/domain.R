...
# if p is present, it is the location of occurrence and used to extract values for env vars from x, a Raster* object
...
if (!all(enviro.data.type=="continuous")) {
    ...
    d = domain(x=occur[,names(current.climate.scenario)])
    # save out the model object
    bccvl.save(d, "model.object.RData")
    # predict for given climate scenario
    domain.proj = predict(d, current.climate.scenario, tails=opt.tails)
    # save output
    bccvl.saveModelProjection(domain.proj, projection.name)
    # evaluate model
    if (!is.null(bkgd)) {
        bccvl.evaluate.model('domain', d, occur, bkgd)
    }
} # end if continuous
