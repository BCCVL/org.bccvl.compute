...
if (model.bioclim) {
    if (!all(enviro.data.type=="continuous")) {
        ...
        if (!is.null(bc)) {
            ...
            bioclim.proj = predict(bc, current.climate.scenario, tails=opt.tails)   # predict for given climate scenario
...
if (evaluate.bioclim) {
    ...
    if (is.null(bioclim.obj)) {
        write(paste("FAIL!", species, "Cannot load bioclim.obj from", wd, "output_bioclim", sep=": "), stdout())
    } else if (is.null(bkgd.data)) {
        write(paste("FAIL!", species, "Cannot evaluate model without bkgd data", wd, "output_bioclim", sep=": "), stdout())
    } else {
