...
setwd(outputdir) # Biomod outputs to workdir
############### BIOMOD2 Models ###############
#
...
glm.BiomodOptions <- list(
	type = "{{ type }}",	#"simple", "quadratic" or "polynomial"; switched off if myFormula is not NULL
	interaction.level = {{ interaction_level }}, #integer corresponding to the interaction level between variables considered; switched off if myFormula is not NULL
	myFormula = NULL, #specific formula; if not NULL, type and interaction.level are args are switched off
	test = "{{ test }}", #"AIC", "BIC" or "none"
	family = "{{ family }}", #"binomial", "gaussian", "gamma", "inverse.gaussian", "poisson", "quasi", "quasibinomial", "quasipoisson"
	mustart = {{ mustart }}, #starting values for the vector of means
	control = list(
		epsilon = {{ control_epsilon }}, #positive convergence tolerance e
		maxit = {{ control_maxit }}, #integer giving the maximal number of IWLS iterations
		trace = {{ 'TRUE' if control_trace else 'FALSE' }} #logical indicating if output should be produced for each iteration
	)
)
