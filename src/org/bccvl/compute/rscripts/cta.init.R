...
cta.BiomodOptions <- list(
	method = {{ method }}, #"anova", "poisson", "class" or "exp"
	...
	control = list(
		xval = {{ control_xval }}, #number of cross-validations
		minbucket = {{ control_minbucket }}, #the minimum number of observations in any terminal <leaf> node
		minsplit = {{ control_minsplit }}, #the minimum number of observations that must exist in a node in order for a split to be attempted
